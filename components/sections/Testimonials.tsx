      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="relative mb-8">
          ...
          >
            {testimonials.map((testimonial, index) => (
              <motion.div
                ...
                className={`relative rounded-2xl overflow-hidden backdrop-blur-2xl border border-white/10 shadow-xl bg-card/50 p-6 sm:p-8 transition-all duration-300 ${showMore || index < 3 ? "" : "hidden lg:block"
                  }`}
              >
                {/* Quote */}
                <p
                  className="text-sm sm:text-base leading-relaxed mb-6 text-foreground/90"
                >
                  {testimonial.quote}
                </p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className="text-sm sm:text-base font-semibold truncate text-foreground"
                      >
                        {testimonial.author.name}
                        {testimonial.author.age && (
                          <span
                            className="font-normal ml-1 text-muted-foreground"
                          >
                            , {testimonial.author.age}
                          </span>
                        )}
                      ...
                      <p
                        className="text-xs sm:text-sm truncate text-muted-foreground"
                      >
          ...
          {!showMore && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="hidden lg:block absolute bottom-0 left-0 right-0 h-80 bg-gradient-to-t from-background via-background/80 to-transparent pointer-events-none"
            />
          )}
        </div>
        ...
        <div className="text-center">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-6 py-3 rounded-full bg-[#2C9F85] hover:bg-[#3CBFA0] text-white font-semibold text-sm sm:text-base transition-colors"
          >
